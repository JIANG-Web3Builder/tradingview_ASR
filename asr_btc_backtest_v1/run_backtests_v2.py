...
from config_v2 import INITIAL_CAPITAL, OUTPUT_DIR, VERSION_ORDER
...
def main() -> None:
    ...
    (OUTPUT_DIR / "data_validation_v2.json").write_text(pd.Series(validation).to_json(indent=2), encoding="utf-8")
    ...
    for version in VERSION_ORDER:
        ...
        equity_df.to_csv(version_dir / "equity_curve_v2.csv", index=False)
        ...
        daily_equity_df.to_csv(version_dir / "daily_equity_v2.csv", index=False)
        trades_df.to_csv(version_dir / "trades_v2.csv", index=False)
        save_summary(version_dir / "summary_v2.json", summary)
        ...
        plot_equity_curve(equity_df, version_dir / "equity_curve_v2.png", f"ASR BTC {version}", subtitle)
        comparison_curves[version] = equity_df

    plot_equity_comparison(comparison_curves, OUTPUT_DIR / "equity_comparison_v2.png")
