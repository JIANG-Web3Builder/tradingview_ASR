...
from config_v8 import BASE_PARAMS, COMMISSION_PCT, INITIAL_CAPITAL, MINTICK, SLIPPAGE_TICKS
from strategy_versions_v8 import get_version_features
...
def run_backtest(data: pd.DataFrame, version: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in data.iterrows():
        ...
        breakout_up_candle_ok = (not features.breakout_candle_confirm) or bullish
        breakout_down_candle_ok = (not features.breakout_candle_confirm) or bearish
        breakout_up = params["breakout_15m"] and is15m and state.trendMode == 0 and breakout_ready and state.breakoutAboveBars >= params["breakout_confirm_bars"] and breakout_up_candle_ok
        breakout_down = params["breakout_15m"] and is15m and state.trendMode == 0 and breakout_ready and state.breakoutBelowBars >= params["breakout_confirm_bars"] and breakout_down_candle_ok
