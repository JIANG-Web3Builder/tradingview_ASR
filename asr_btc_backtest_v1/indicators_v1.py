...
from config_v1 import BASE_PARAMS, MINTICK, TIMEFRAME_MINUTES
...
def compute_indicators(df: pd.DataFrame, base_params: dict | None = None) -> pd.DataFrame:
    ...
    out["midLine"] = ((out["high"] + out["low"]) / 2.0).rolling(hl_length, min_periods=hl_length).mean()
    ...
    out["dynamicOffset"] = (out["midLine"] * params["base_width_pct"] / 100.0 / 2.0) * (1.0 + out["vovSafe"] * params["adjust_factor"])
    ...
    out["midHigh"] = (out["smoothMid"] + out["smoothRes"]) / 2.0
    out["midLow"] = (out["smoothMid"] + out["smoothSup"]) / 2.0
