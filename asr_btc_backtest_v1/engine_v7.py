...
from config_v7 import BASE_PARAMS, COMMISSION_PCT, INITIAL_CAPITAL, MINTICK, SLIPPAGE_TICKS
from strategy_versions_v7 import get_version_features
...
def run_backtest(data: pd.DataFrame, version: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in data.iterrows():
        ...
        long1_pos = state.open_positions.get("Long1")
        short1_pos = state.open_positions.get("Short1")
        long2_low_progress = long1_pos is not None and long1_pos.mfe_abs < float(row["zoneOffset"])
        short2_low_progress = short1_pos is not None and short1_pos.mfe_abs < float(row["zoneOffset"])
        long2_alloc_pct = params["alloc_pct2"] * 0.5 if features.low_progress_l2_half_size and long2_low_progress else params["alloc_pct2"]
        short2_alloc_pct = params["alloc_pct2"] * 0.5 if features.low_progress_l2_half_size and short2_low_progress else params["alloc_pct2"]

        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_flat() and cooldown_ok and (not long1_be_lock) and long_ma_filter_ok and near_mid_low and bullish and float(row["rsiVal"]) < params["rsi_long"]:
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_long() and state.nLong == 1 and cooldown_ok and (not long2_be_lock) and near_sup and float(row["rsiVal"]) < params["rsi_long"] + 10 and (not features.deep_entry_reclaim or (long2_overshoot_ok and long2_reclaim_ok)):
            qty = state.current_equity(close) * (long2_alloc_pct / 100.0) / close
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_long() and state.nLong == 2 and cooldown_ok and (not long3_be_lock) and near_downer and float(row["rsiVal"]) < params["rsi_long"] + 10 and (not features.deep_entry_reclaim or (long3_overshoot_ok and long3_reclaim_ok)):
            ...
            state.lastEntryBar = int(row["bar_index"])
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_flat() and cooldown_ok and (not short1_be_lock) and params["enable_short"] and short_ma_filter_ok and near_mid_high and bearish and float(row["rsiVal"]) > params["rsi_short"]:
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_short() and state.nShort == 1 and cooldown_ok and (not short2_be_lock) and near_res and float(row["rsiVal"]) > params["rsi_short"] - 10 and (not features.deep_entry_reclaim or (short2_overshoot_ok and short2_reclaim_ok)):
            qty = state.current_equity(close) * (short2_alloc_pct / 100.0) / close
