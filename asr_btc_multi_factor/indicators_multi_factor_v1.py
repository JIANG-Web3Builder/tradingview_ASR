...
from config_multi_factor_v1 import BASE_PARAMS, MINTICK, TIMEFRAME_MINUTES
...
def compute_indicators(df: pd.DataFrame, base_params: dict | None = None) -> pd.DataFrame:
    ...
    out["rsiDelta"] = out["rsiVal"].diff(3)
    out["return4"] = out["close"].pct_change(4)
    out["return16"] = out["close"].pct_change(16)
    out["volumeSma20"] = out["volume"].rolling(20, min_periods=20).mean()
    out["volumeRatio20"] = out["volume"] / out["volumeSma20"].replace(0.0, np.nan)
    out["atrPctRank"] = out["atrPct"].rolling(vov_length, min_periods=vov_length).rank(pct=True)
