...
from config_multi_factor_v1 import BASE_PARAMS, COMMISSION_PCT, INITIAL_CAPITAL, MINTICK, SLIPPAGE_TICKS
...
def _row_float(row: pd.Series, column: str, fallback: float) -> float:
...
def run_backtest(data: pd.DataFrame, version: str) -> tuple[pd.DataFrame, pd.DataFrame]:
    ...
    for _, row in data.iterrows():
        ...
        bullish = close > open_
        bearish = close < open_
        rsi_val = float(row["rsiVal"])
        rsi_delta = _row_float(row, "rsiDelta", 0.0)
        return4 = _row_float(row, "return4", 0.0)
        return16 = _row_float(row, "return16", 0.0)
        volume_ratio = _row_float(row, "volumeRatio20", 1.0)
        atr_pct_rank = _row_float(row, "atrPctRank", 0.5)
        trend_volume_ok = (not features.trend_entry_volume_filter) or volume_ratio >= 1.05
        breakout_up_direction_ok = (not features.breakout_direction_filter) or (return4 >= 0.002 and rsi_delta >= -2.0)
        breakout_down_direction_ok = (not features.breakout_direction_filter) or (return4 <= -0.002 and rsi_delta <= 2.0)
        ...
        breakout_up = params["breakout_15m"] and is15m and state.trendMode == 0 and breakout_ready and trend_volume_ok and breakout_up_direction_ok and state.breakoutAboveBars >= params["breakout_confirm_bars"]
        breakout_down = params["breakout_15m"] and is15m and state.trendMode == 0 and breakout_ready and trend_volume_ok and breakout_down_direction_ok and state.breakoutBelowBars >= params["breakout_confirm_bars"]
        ...
        if not did_breakout_flip and state.trendMode == 1:
            ...
            breakout_long_early_fail = features.breakout_early_fail_exit and state.breakoutEntryBar is not None and (int(row["bar_index"]) - state.breakoutEntryBar) >= max(1, params["breakout_min_hold_bars"] // 2) and close <= float(row["smoothRes"])
            breakout_long_pos = state.open_positions.get("TrendLong")
            breakout_long_no_progress = features.breakout_no_progress_exit and breakout_long_pos is not None and state.breakoutEntryBar is not None and (int(row["bar_index"]) - state.breakoutEntryBar) >= params["breakout_min_hold_bars"] and close <= breakout_long_pos.entry_price and breakout_long_pos.mfe_abs < float(row["zoneOffset"])
            breakout_long_stop = state.breakoutInsideBars >= params["breakout_back_bars"] or breakout_long_early_fail or breakout_long_no_progress
            if breakout_long_tp:
                qty = state.current_equity(close) / close
                state.trendMode = -2
                state.breakoutInsideBars = 0
        ...
        if not did_breakout_flip and state.trendMode == -1:
            ...
            breakout_short_early_fail = features.breakout_early_fail_exit and state.breakoutEntryBar is not None and (int(row["bar_index"]) - state.breakoutEntryBar) >= max(1, params["breakout_min_hold_bars"] // 2) and close >= float(row["smoothSup"])
            breakout_short_pos = state.open_positions.get("TrendShort")
            breakout_short_no_progress = features.breakout_no_progress_exit and breakout_short_pos is not None and state.breakoutEntryBar is not None and (int(row["bar_index"]) - state.breakoutEntryBar) >= params["breakout_min_hold_bars"] and close >= breakout_short_pos.entry_price and breakout_short_pos.mfe_abs < float(row["zoneOffset"])
            breakout_short_stop = state.breakoutInsideBars >= params["breakout_back_bars"] or breakout_short_early_fail or breakout_short_no_progress
        ...
        def early_no_progress_check(entry_id: str, ent_bar: int, ep: float, is_buy: bool) -> bool:
            if not features.early_no_progress_time_stop:
                return False
            if _level_from_entry_id(entry_id) != 1:
                return False
            pos = state.open_positions.get(entry_id)
            if pos is None:
                return False
            bars_held = int(row["bar_index"]) - ent_bar
            profitable = close > ep if is_buy else close < ep
            review_bars = max(1, int(row["timeStopBars"]) // 2)
            return bars_held >= review_bars and not profitable and pos.mfe_abs < float(row["zoneOffset"])

        def l1_adverse_fail_check(entry_id: str, ent_bar: int, is_buy: bool) -> bool:
            if not features.l1_adverse_fail_exit:
                return False
            if _level_from_entry_id(entry_id) != 1:
                return False
            pos = state.open_positions.get(entry_id)
            if pos is None:
                return False
            bars_held = int(row["bar_index"]) - ent_bar
            review_bars = max(1, int(row["timeStopBars"]) // 4)
            adverse_abs = max(0.0, -pos.mae_abs)
            material_adverse = adverse_abs >= max(float(row["zoneOffset"]), pos.entry_price * 0.005)
            failed_structure = close < float(row["smoothSup"]) if is_buy else close > float(row["smoothRes"])
            return bars_held >= review_bars and material_adverse and failed_structure and pos.mfe_abs < float(row["zoneOffset"])

        def deep2_adverse_fail_check(entry_id: str, ent_bar: int, is_buy: bool) -> bool:
            if not features.deep2_adverse_fail_exit:
                return False
            if _level_from_entry_id(entry_id) != 2:
                return False
            pos = state.open_positions.get(entry_id)
            if pos is None:
                return False
            ...
            review_bars = max(1, int(row["timeStopBars"]) // 4)
            adverse_abs = max(0.0, -pos.mae_abs)
            material_adverse = adverse_abs >= max(float(row["zoneOffset"]), pos.entry_price * 0.0075)
            failed_structure = close < float(row["downerLine"]) if is_buy else close > float(row["uperLine"])
            return bars_held >= review_bars and material_adverse and failed_structure and pos.mfe_abs < float(row["zoneOffset"])

        def deep2_reclaim_fail_check(entry_id: str, ent_bar: int, is_buy: bool) -> bool:
            if not features.deep2_reclaim_fail_exit:
                return False
            if _level_from_entry_id(entry_id) != 2:
                return False
            pos = state.open_positions.get(entry_id)
            if pos is None:
                return False
            bars_held = int(row["bar_index"]) - ent_bar
            review_bars = max(1, int(row["timeStopBars"]) // 8)
            failed_reclaim = close < float(row["smoothSup"]) if is_buy else close > float(row["smoothRes"])
            adverse_abs = max(0.0, -pos.mae_abs)
            material_adverse = adverse_abs >= max(float(row["zoneOffset"]) * 0.75, pos.entry_price * 0.005)
            return bars_held >= review_bars and failed_reclaim and material_adverse and pos.mfe_abs < float(row["zoneOffset"])
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and state.is_long() and state.entryPx1 is not None:
            ...
            if l1_adverse_fail_check("Long1", state.entryBar1, True):
                _close_position(state, "Long1", row, close, "L1AdverseFail L1", "L1AdverseFail L1", "market")
            elif early_no_progress_check("Long1", state.entryBar1, state.entryPx1, True):
                _close_position(state, "Long1", row, close, "EarlyNoProgress L1", "EarlyNoProgress L1", "market")
            elif no_progress_check("Long1", state.entryBar1, state.entryPx1, True):
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and state.is_long() and state.entryPx2 is not None:
            ...
            if deep2_reclaim_fail_check("Long2", state.entryBar2, True):
                _close_position(state, "Long2", row, close, "Deep2ReclaimFail L2", "Deep2ReclaimFail L2", "market")
            elif deep2_adverse_fail_check("Long2", state.entryBar2, True):
                _close_position(state, "Long2", row, close, "Deep2AdverseFail L2", "Deep2AdverseFail L2", "market")
            elif no_progress_check("Long2", state.entryBar2, state.entryPx2, True):
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and state.is_short() and state.entryPx1 is not None:
            ...
            if l1_adverse_fail_check("Short1", state.entryBar1, False):
                _close_position(state, "Short1", row, close, "L1AdverseFail S1", "L1AdverseFail S1", "market")
            elif early_no_progress_check("Short1", state.entryBar1, state.entryPx1, False):
                _close_position(state, "Short1", row, close, "EarlyNoProgress S1", "EarlyNoProgress S1", "market")
            elif no_progress_check("Short1", state.entryBar1, state.entryPx1, False):
        ...
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and state.is_short() and state.entryPx2 is not None:
            ...
            if deep2_reclaim_fail_check("Short2", state.entryBar2, False):
                _close_position(state, "Short2", row, close, "Deep2ReclaimFail S2", "Deep2ReclaimFail S2", "market")
            elif deep2_adverse_fail_check("Short2", state.entryBar2, False):
                _close_position(state, "Short2", row, close, "Deep2AdverseFail S2", "Deep2AdverseFail S2", "market")
            elif no_progress_check("Short2", state.entryBar2, state.entryPx2, False):
        ...
        if features.mfe_zone_breakeven and state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit:
            ...
            for entry_id, level in [("Long1", 1), ("Long2", 2), ("Long3", 3)]:
                ...
                if pos is None:
                    continue
                protect_threshold = zone_offset * features.l1_mfe_protect_multiplier if level == 1 else zone_offset
                protect_price = pos.entry_price * (1.0 + params["be_buffer"])
                if pos.mfe_abs >= protect_threshold and close <= protect_price:
        ...
        if features.level0_mfe_zone_breakeven and not did_breakout_flip and not did_breakout_exit:
            ...
            for entry_id in ["TrendLong", "RevLong"]:
                ...
                can_protect = can_protect or (entry_id == "RevLong" and (not reverse_lock_active or features.reverse_mfe_protect_during_lock))
            ...
            for entry_id in ["TrendShort", "RevShort"]:
                ...
                can_protect = can_protect or (entry_id == "RevShort" and (not reverse_lock_active or features.reverse_mfe_protect_during_lock))
                if can_protect and pos.mfe_abs >= zone_offset and close >= protect_price:
                    _close_position(state, entry_id, row, close, f"MFEProtect {entry_id}", f"MFEProtect {entry_id}", "market")
                    state.breakoutInsideBars = 0
                    state.breakoutEntryBar = None
        ...
        long1_alloc_pct *= features.long1_alloc_multiplier
        short1_alloc_pct *= features.short1_alloc_multiplier
        deep_alloc_pct2 = params["alloc_pct2"] * features.deep_alloc_multiplier
        deep_alloc_pct3 = params["alloc_pct3"] * features.deep_alloc_multiplier
        short1_trend_quality_ok = (not features.short1_downtrend_filter) or bool(row["downtrend"])
        l1_long_rsi_ok = features.l1_rsi_long_max is None or rsi_val <= features.l1_rsi_long_max
        l1_short_rsi_ok = features.l1_rsi_short_min is None or rsi_val >= features.l1_rsi_short_min
        deep_long_rsi_ok = features.deep_rsi_long_max is None or rsi_val <= features.deep_rsi_long_max
        deep_short_rsi_ok = features.deep_rsi_short_min is None or rsi_val >= features.deep_rsi_short_min
        long_l1_slope_ok = (not features.l1_rsi_slope_confirm) or rsi_delta >= 0.0
        short_l1_slope_ok = (not features.l1_rsi_slope_confirm) or rsi_delta <= 0.0
        long_l1_antirunaway_ok = (not features.l1_antirunaway_filter) or (return4 >= -0.015 and return16 >= -0.04)
        short_l1_antirunaway_ok = (not features.l1_antirunaway_filter) or (return4 <= 0.015 and return16 <= 0.04)
        l1_volume_ok = (not features.l1_volume_confirm) or volume_ratio >= 0.9
        channel_volatility_ok = (not features.high_volatility_filter) or atr_pct_rank <= 0.9
        long_l1_factor_ok = l1_long_rsi_ok and long_l1_slope_ok and long_l1_antirunaway_ok and l1_volume_ok and channel_volatility_ok
        short_l1_factor_ok = l1_short_rsi_ok and short_l1_slope_ok and short_l1_antirunaway_ok and l1_volume_ok and channel_volatility_ok
        deep_long_factor_ok = deep_long_rsi_ok and channel_volatility_ok
        deep_short_factor_ok = deep_short_rsi_ok and channel_volatility_ok

        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_flat() and cooldown_ok and (not long1_be_lock) and long_ma_filter_ok and near_mid_low and long1_reclaim_entry_ok and long_l1_factor_ok and bullish and rsi_val < params["rsi_long"]:
            ...
            state.lastEntryBar = int(row["bar_index"])
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_long() and state.nLong == 1 and cooldown_ok and (not long2_be_lock) and near_sup and deep_long_factor_ok and rsi_val < params["rsi_long"] + 10 and (not features.deep_entry_reclaim or (long2_overshoot_ok and long2_reclaim_ok)):
            qty = state.current_equity(close) * (deep_alloc_pct2 / 100.0) / close
            _enter_position(state, "Long2", "long", qty, row, 2)
            state.nLong = 2
            state.set_level_entry_fields(2, close, int(row["bar_index"]))
            state.lastEntryBar = int(row["bar_index"])
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_long() and state.nLong == 2 and cooldown_ok and (not long3_be_lock) and near_downer and deep_long_factor_ok and rsi_val < params["rsi_long"] + 10 and (not features.deep_entry_reclaim or (long3_overshoot_ok and long3_reclaim_ok)):
            qty = state.current_equity(close) * (deep_alloc_pct3 / 100.0) / close
            _enter_position(state, "Long3", "long", qty, row, 3)
            state.nLong = 3
            state.set_level_entry_fields(3, close, int(row["bar_index"]))
            state.lastEntryBar = int(row["bar_index"])
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_flat() and cooldown_ok and (not short1_be_lock) and params["enable_short"] and short_ma_filter_ok and short1_trend_quality_ok and near_mid_high and short1_reclaim_entry_ok and short_l1_factor_ok and bearish and rsi_val > params["rsi_short"]:
            qty = state.current_equity(close) * (short1_alloc_pct / 100.0) / close
            _enter_position(state, "Short1", "short", qty, row, 1)
            state.nShort = 1
            state.set_level_entry_fields(1, close, int(row["bar_index"]))
            state.lastEntryBar = int(row["bar_index"])
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_short() and state.nShort == 1 and cooldown_ok and (not short2_be_lock) and near_res and deep_short_factor_ok and rsi_val > params["rsi_short"] - 10 and (not features.deep_entry_reclaim or (short2_overshoot_ok and short2_reclaim_ok)):
            qty = state.current_equity(close) * (deep_alloc_pct2 / 100.0) / close
            _enter_position(state, "Short2", "short", qty, row, 2)
            state.nShort = 2
            state.set_level_entry_fields(2, close, int(row["bar_index"]))
            state.lastEntryBar = int(row["bar_index"])
        if state.trendMode == 0 and not did_breakout_flip and not did_breakout_exit and not did_fast_exit and state.is_short() and state.nShort == 2 and cooldown_ok and (not short3_be_lock) and near_uper and deep_short_factor_ok and rsi_val > params["rsi_short"] - 10 and (not features.deep_entry_reclaim or (short3_overshoot_ok and short3_reclaim_ok)):
            qty = state.current_equity(close) * (deep_alloc_pct3 / 100.0) / close
