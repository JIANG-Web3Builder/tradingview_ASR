...
OUTPUT_DIR = BASE_DIR / "output_channel_eth_v1"
...
from indicators_channel_v5 import compute_indicators
...
def plot_equity_comparison(curves: dict[str, pd.DataFrame], output_path: Path) -> None:
    ...
    ax.set_title("ASR ETH Equity Curve Comparison (v10-v30)")
    ax.set_xlabel("Time")
    ax.set_ylabel("Equity")
    ax.legend(ncols=2)
    ax.grid(True, alpha=0.3)
...
def main() -> None:
    ...
    (OUTPUT_DIR / "data_validation_channel_eth_v1.json").write_text(pd.Series(validation).to_json(indent=2), encoding="utf-8")
    ...
    for version in VERSION_ORDER:
        ...
        equity_df.to_csv(version_dir / "equity_curve_channel_eth_v1.csv", index=False)
        ...
        daily_equity_df.to_csv(version_dir / "daily_equity_channel_eth_v1.csv", index=False)
        trades_df.to_csv(version_dir / "trades_channel_eth_v1.csv", index=False)
        save_summary(version_dir / "summary_channel_eth_v1.json", summary)
        ...
        plot_equity_curve(equity_df, version_dir / "equity_curve_channel_eth_v1.png", f"ASR ETH {version}", subtitle)
        comparison_curves[version] = equity_df

    plot_equity_comparison(comparison_curves, OUTPUT_DIR / "equity_comparison_channel_eth_v1.png")
    pd.DataFrame(summary_rows).sort_values("version").to_csv(OUTPUT_DIR / "summary_comparison_channel_eth_v1.csv", index=False)
