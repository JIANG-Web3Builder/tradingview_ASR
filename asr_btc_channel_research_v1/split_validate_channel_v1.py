...
BASE = Path("output_channel_v3")
VERSIONS = ["v17", "v17_ch1", "v17_ch2", "v17_ch3"]
PERIODS = [
...
def main() -> None:
    ...
    for version in VERSIONS:
        equity_df = pd.read_csv(BASE / version / "equity_curve_channel_v3.csv")
        trades_df = pd.read_csv(BASE / version / "trades_channel_v3.csv")
        summary = pd.read_json(BASE / version / "summary_channel_v3.json", typ="series")
    ...
    result.to_csv(BASE / "split_validate_channel_v1.csv", index=False)
    print("=== Return % ===")
